...
import { getFundingProgress, selectPublishedProposals, type PublicProposal } from "@/lib/publicProposals";
            ...
            {isLoadingShelf ? (
              ...
              <div className="mx-auto mt-10 grid max-w-6xl gap-5 sm:mt-12 md:grid-cols-2">
                {publishedProposals.map((proposal) => {
                  ...
                  return <article key={proposal.id} className="overflow-hidden rounded-[30px] border border-[#e8e8ed] bg-[#f5f5f7] text-left shadow-[0_14px_45px_rgba(20,20,35,0.05)]">
                    <ProposalCover coverUrl={proposal.coverUrl} title={proposal.title} />
                    <div className="p-6 sm:p-7"><div className="flex items-center justify-between gap-3"><span className="rounded-full bg-[#fff3e8] px-2.5 py-1 text-xs font-semibold text-[#bd4d21]">已公開</span><span className="text-xs text-[#6e6e73]">{proposal.category || "作品提案"}</span></div><h3 className="font-display mt-5 text-3xl tracking-[-0.04em] text-[#1d1d1f]">{proposal.title || "未命名作品"}</h3><p className="mt-2 text-sm text-[#6e6e73]">{proposal.creatorName || "老東家創作者"}</p><p className="mt-5 text-sm leading-7 text-[#424245]">{proposal.summary || proposal.description || "創作者正在整理這份作品的公開說明。"}</p><div className="mt-6 rounded-2xl bg-white p-4"><div className="flex items-end justify-between gap-4"><p className="text-sm text-[#6e6e73]">已募得 <span className="font-semibold text-[#1d1d1f]">NT$ {funding.raised.toLocaleString("zh-TW")}</span></p><p className="text-xl font-semibold text-[#d95f35]">{funding.percentage}%</p></div><div className="mt-3 h-2 overflow-hidden rounded-full bg-[#e5e5e7]" role="progressbar" aria-label={`${proposal.title || "作品"}募資進度`} aria-valuemin={0} aria-valuemax={100} aria-valuenow={funding.barPercentage}><div className="h-full rounded-full bg-[#f36b3b] transition-[width] duration-500" style={{ width: `${funding.barPercentage}%` }} /></div><p className="mt-3 text-xs text-[#6e6e73]">作品目標 NT$ {funding.target.toLocaleString("zh-TW")}</p></div><div className="mt-3 grid grid-cols-2 gap-3"><div className="rounded-2xl bg-white p-4"><p className="text-xs text-[#6e6e73]">支持目標</p><p className="mt-1 font-semibold text-[#1d1d1f]">NT$ {funding.target.toLocaleString("zh-TW")}</p></div><div className="rounded-2xl bg-white p-4"><p className="text-xs text-[#6e6e73]">最低支持</p><p className="mt-1 font-semibold text-[#1d1d1f]">NT$ {(Number(proposal.minimumSupportAmount) || 0).toLocaleString("zh-TW")}</p></div></div><dl className="mt-5 space-y-3 border-t border-[#e5e5e7] pt-5 text-sm"><div className="flex gap-3"><dt className="w-20 shrink-0 text-[#6e6e73]">製作階段</dt><dd className="text-[#1d1d1f]">{proposal.currentStage || "即將開始"}</dd></div><div className="flex gap-3"><dt className="w-20 shrink-0 text-[#6e6e73]">下一里程碑</dt><dd className="text-[#1d1d1f]">{proposal.nextMilestone || "待公布"}</dd></div><div className="flex gap-3"><dt className="w-20 shrink-0 text-[#6e6e73]">預計完成</dt><dd className="text-[#1d1d1f]">{proposal.estimatedCompletion || "待公布"}</dd></div></dl></div>
                  </article>;
                })}
              </div>
            ) : (
