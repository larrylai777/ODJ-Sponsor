    ...
    <header className="sticky top-0 z-40 border-b paper-rule bg-[#fbf8ef]/92 backdrop-blur-md">
      <div className="container flex h-[72px] items-center justify-between gap-6">
        <a href="/" className="group flex items-center gap-3" aria-label="回到老東家首頁">
          <img className="h-9 w-9 transition-transform duration-200 group-hover:rotate-3" src="/manus-storage/odj-logo-seal_44c2c579.png" alt="老東家朱紅藏書印" />
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => toast("提案入口示意", { description: "下一步可串接創作者登入、提案審核與周邊工坊報價。" })} className="hidden border border-[#b84236] px-4 py-2 text-sm font-semibold text-[#a83932] transition hover:bg-[#b84236] hover:text-[#fff8ee] sm:block">我要提案</button>
          <button onClick={onMenu} className="grid h-10 w-10 place-items-center border paper-rule md:hidden" aria-label="開啟選單"><Menu size={19} /></button>
        </div>
      </div>
    </header>
  );
...
export default function Home() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden">
          ...
          <div className="container grid min-h-[620px] items-stretch lg:grid-cols-[0.95fr_1.05fr]">
            <div className="relative z-10 flex flex-col justify-center py-20 lg:py-24">
              ...
              <div className="mt-10 flex flex-wrap gap-3">
                <a href="#shelf" className="group inline-flex items-center gap-3 bg-[#b84236] px-6 py-3.5 text-sm font-bold text-[#fff8ee] transition duration-200 hover:-translate-y-0.5 hover:bg-[#963128] active:scale-[0.97]">逛逛今日書架 <ArrowDownRight size={17} className="transition group-hover:translate-y-0.5 group-hover:translate-x-0.5" /></a>
                <button onClick={() => toast("平台原型", { description: "此版本展示小說與周邊群募的發現、詳情與方案選擇流程。" })} className="inline-flex items-center gap-2 border border-[#4f4740] px-6 py-3.5 text-sm font-bold transition hover:bg-[#f7f0e1] active:scale-[0.97]">先認識老東家 <ChevronRight size={16} /></button>
              </div>
      ...
      <footer className="border-t paper-rule bg-[#f0e7d7]"><div className="container flex flex-col justify-between gap-6 py-9 sm:flex-row sm:items-end"><div className="flex items-center gap-3"><img className="h-9 w-9" src="/manus-storage/odj-logo-seal_44c2c579.png" alt=""/><div><p className="font-display text-lg font-black tracking-[0.08em]">老東家</p><p className="mt-1 text-xs text-[#70645a]">故事得有人先相信。</p></div></div><div className="text-xs leading-6 text-[#756a60]"><p>ODJ Sponsor Prototype · 僅供產品概念展示</p><p>不處理真實付款、訂單或個人資料。</p></div></div></footer>
    </div>
  );
}
